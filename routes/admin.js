        ...
        try {
            ...
            try {
                ...
                await sendEmail(
                    ...
                    `Delivery Partner Assigned - YM${String(orderId).padStart(
                        5,
                        "0"
                    )}`,
                    `
<div style="font-family:'Segoe UI', Arial; background:#f4f6fb; padding:40px 20px;">
  <div style="max-width:580px; margin:auto; background:#ffffff; padding:35px; border-radius:16px; box-shadow:0 10px 30px rgba(0,0,0,0.08);">
    ...
    app.get("/admin/restaurant-applications", isAdmin, async (req, res) => {
        try {
            const { status } = req.query;
            let query = `SELECT * FROM restaurant_applications`;
            const params = [];
            ...
            console.error(err);
            res.status(500).json({
                error: "Failed to fetch restaurant applications",
            });
        }
    });
        ...
        async (req, res) => {
            try {
                ...
                if (
                    ...
                    return res
                        .status(400)
                        .json({ error: "Application is already processed" });
                }
                await getPool().query(
   ...
   VALUES (?, ?, ?, ?, 0, NOW())`,
    ...
    fssai, gst, pan,
    status, is_approved, rating, total_orders
  )
  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
                    [
                        application.owner_id,
...
<div style="font-family:'Segoe UI', Arial; background:#f4f6fb; padding:40px 20px;">
  <div style="max-width:580px; margin:auto; background:#ffffff; padding:35px; border-radius:16px; box-shadow:0 10px 30px rgba(0,0,0,0.08);">
    <h2 style="margin:0; color:#E53935;">Application approved</h2>
    <p style="color:#555; line-height:1.6; margin-top:16px;">Hi ${application.owner_name}, your restaurant application for ${application.restaurant_name} has been approved. Your restaurant is now live in the system.</p>
    <p style="color:#555; margin-top:16px;">You can now sign in and manage your restaurant from the dashboard.</p>
  </div>
</div>
                    `
                    );
                ...
                res.json({ success: true, restaurant: created[0] });
            } catch (err) {
                console.error(err);
                res.status(500).json({
    ...
    app.put(
        "/admin/delivery-boys/:id/availability",
        isAdmin,
        async (req, res) => {
            try {
                await ensureAvailabilityColumn();
